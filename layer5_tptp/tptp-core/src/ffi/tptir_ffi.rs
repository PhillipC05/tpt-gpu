...
/// Safe wrapper around TPTIR compiler context
...
/// Safe wrapper around a TPTIR module
...
/// Compile TPTIR source to the specified target
...
/// Get the TPTIR library version
pub fn get_tptir_version() -> String { let (major, minor, patch) = TptirCompilerHandle::version(); format!("{}.{}.{}", major, minor, patch) }

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn test_version() { let version = get_tptir_version(); assert!(!version.is_empty()); }
    #[test] fn test_status_string() { let ok_str = TptirCompilerHandle::status_string(TptirStatus::Ok); assert_eq!(ok_str, "OK"); }
}